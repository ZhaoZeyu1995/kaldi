...
cat $phone_dir/lexicon_raw_nosil.txt | \
  perl -e 'while(<>){@A = split; if(! $seen{$A[0]}) {$seen{$A[0]} = 1; print $_;}}' \
       > $phone_dir/lexicon1_raw_nosil.txt || exit 1;

cat $phone_dir/lexicon1_raw_nosil.txt | awk '{$1=$1; print}' FS='[ \t ]' OFS=' ' > $phone_dir/lexicon2_raw_nosil.txt
