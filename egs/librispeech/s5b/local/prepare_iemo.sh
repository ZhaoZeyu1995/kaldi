...
trans=$src_dir/IEM.txt
original_trans=$src_dir/emo_name_trans1.txt
output_dir=iemo
...
awk -F, '{print $2}' $trans > $local_dir/utts

awk '{print $1 " /disk/scratch3/zzhao/data/IEMOCAP/Data/" $1 ".wav" }' $local_dir/utts | sort > $data_dir/wav.scp

awk '{split($2, a, "."); $1=$2=""; print a[1] toupper($0)}' $original_trans | sort > $data_dir/text

awk '{split($2, a, ".");  split(a[1], b, "_"); print a[1] " " b[1]}' $original_trans | sort > $data_dir/utt2spk
